...
3. **Model collision-bound extraction (Research).** Trees and explicit point obstructions now receive conservative route-proximity warnings, and collision-enabled props are inventoried in previews. Recover or cache trustworthy per-model physical bounds before allowing props to alter offline reachability; never substitute render bounds without clearly marking the approximation.
4. **Minimap image verification (Human visual judgment).** Compare generated bounds and camera transforms with an in-game screenshot and add a golden fixture after alignment is confirmed.
5. **CI-friendly licensed map fixture (Repository decision).** Check in or generate a small legal VMAP fixture so compile/conversion tests can run on machines with Workshop Tools without depending on the private 3v3 project.
