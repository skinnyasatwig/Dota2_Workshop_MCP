...
3. **CI-friendly licensed map fixture (Repository decision).** Check in or generate a small legal VMAP fixture so compile/conversion tests can run on machines with Workshop Tools without depending on the private 3v3 project.
...
5. **Model collision refinement (Research).** Base-game props now use cached bounds recovered only from real model `PHYS` blocks. Add addon-archive lookup, pitched/rolled transforms, and—only if VRF exposes a stable representation—exact convex hull surfaces. Dynamic and team-selective collision remain engine-only facts.
