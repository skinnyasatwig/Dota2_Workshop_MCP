...
1. **Richer world geometry (Valve-format research).** Flat or per-corner-sloped convex/concave solid extrusions, flat/sloped convex gameplay volumes, segmented player-blocking walls, rectangular arches, checked navigation surfaces, bridge decks/approaches, regular rings, and matching-point irregular holed platforms are safe. The practical ring case is compiler- and GridNav-proven; the irregular five-segment case is compiler-proven. Next, research unmatched boundary counts or multiple holes, irregular arches, and curated decorative material composition without exposing unsafe triangle soup. The bridge fixture also proved that same-X/Y points at different heights alias in GridNav; do not promise independent stacked bridge and underpass routes unless a different Valve-supported representation is found.
