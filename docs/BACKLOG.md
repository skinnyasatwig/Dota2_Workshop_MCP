...
3. **Renderer-safe screenshot evidence (Research).** Live minimap coordinates are proven, but `PrintWindow` returned a black frame on the acceptance machine and per-probe capture can disturb Panorama timing. Keep measurement-only mode as the reliable default for calibration; add a checked non-black capture path before calling screenshots portable.
