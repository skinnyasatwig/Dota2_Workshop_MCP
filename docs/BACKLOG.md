...
1. **Richer world geometry (Valve-format research).** Flat or per-corner-sloped convex/concave solid extrusions, flat/sloped convex gameplay volumes, segmented player-blocking wall outlines, rectangular three-piece arches, checked Valve navigation surfaces, bridge decks/approaches, and pre-compile material existence checks are safe. Next, research true holes or irregular arches and curated decorative material composition without exposing arbitrary unsafe triangle soup. The bounded bridge fixture proved an elevated surface route, but same-X/Y points at different heights alias in real GridNav; do not promise independent stacked bridge and underpass routes unless a different Valve-supported representation is found.
