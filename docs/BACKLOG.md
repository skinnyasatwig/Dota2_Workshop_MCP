...
4. **Model collision refinement (Research).** Loose and packed addon props plus base-game props now use cached bounds recovered only from real model `PHYS` blocks and full pitch/yaw/roll transforms. Only if VRF exposes a stable representation, replace conservative projected bound hulls with exact convex collision surfaces. Dynamic and team-selective collision remain engine-only facts.
