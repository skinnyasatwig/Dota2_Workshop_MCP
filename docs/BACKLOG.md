...
4. **Model collision completeness (Research).** Identity-part convex `RnHull_t` shapes now use validated exact vertices, while non-empty bind-pose hulls and PHYS meshes retain conservative bounds. Pair per-part bind poses safely and evaluate stable vertex access for mesh, sphere, and capsule shapes without weakening the truthful fallback. Dynamic and team-selective collision remain engine-only facts.
...
6. **Baseline refresh workflow (Maintenance).** When recipe-version checks detect a real Valve change, provide a guided re-decompile/re-test report before a maintainer intentionally records new hashes; never auto-bless an update.
