...
1. **Minimap image verification (Human visual judgment).** Metadata, source/compiled assets, PNG dimensions, and the unrotated world transform are now checked offline. Compare the rendered artwork/camera behavior with an in-game screenshot and add a golden fixture after visual alignment is confirmed.
