...
| Checked solid volumes | Ready for rectangular recipes | Camp bounds, general/hero/Dota triggers, boss-attack areas, no-ward zones, and player blockers can be generated, mirrored, inspected, validated, and repaired from JSON. Valve's converter round-trip is tested. | Arbitrary classes and polygonal/curved solids are rejected; base/FoW point blockers use separate Valve entities. |
...
| Diagnostic preview | Ready | A top-down image shows terrain, contours, cliffs, ramps, water/currents, paths, entities, tower ranges, camps, objectives, minimap bounds, checked volumes, holes, and unreachable areas. | It is a diagnostic drawing, not the final in-game rendering. |
| Offline reachability | Ready | The tile grid can find holes, trapped spawns, isolated land, blocked entrances, inaccessible objectives, bad path segments, and checked player-blocker footprints without launching Dota. | Props, trees, other mesh collision, and Valve's compiled navigation are engine facts. |
...
| Reusable Dota structures | Ready, including camp boxes | Bases, Ancients, towers, fountains, shops, camps, boss pits, player starts, and gates have checked builders and stock names. Camps can create their real spawn boundary. | Circular boss-pit triggers and bespoke walls still need polygonal solid research or Hammer. |
...
Most layout iteration can now happen in JSON plus generated previews. Hammer is still the right tool for final visual dressing, arbitrary mesh sculpting, non-rectangular solids, and visual inspection. The goal is not to pretend Hammer no longer exists; it is to reserve Hammer for the parts only Hammer can reliably judge.
